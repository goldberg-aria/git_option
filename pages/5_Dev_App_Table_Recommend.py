...
st.set_page_config(page_title="옵션 전략 추천 (실전 시뮬레이션)", layout="centered")
st.title("실전 옵션 전략 추천 (Option Chain 기반)")
...
try:
    ...
    # 전략별 시뮬레이션 결과 자동 출력
    전략목록 = ["롱콜", "숏풋", "콜스프레드", "커버드콜"]
    전략코드 = ["long_call", "short_put", "vertical_call_spread", "covered_call"]
    results = []
    ...
    st.info(f"승률 산출 기준: 매수(롱) 전략은 프리미엄(투자금) 기준, 매도(숏) 전략은 증거금(행사가/기초자산×100×40%) 기준. 입력한 목표수익률(%) + 거래비용(%)을 해당 기준에 곱해, 그 이상 순수익 달성 시 '승'으로 간주합니다. (40%는 미국 옵션 마진 규정의 예시값)")
    ...
    <b>승률 산출 기준 안내</b><br>
    - <b>매수(롱) 전략</b> (롱콜, 콜스프레드): <b>프리미엄(투자금)</b> 기준<br>
    - <b>매도(숏) 전략</b> (숏풋, 커버드콜): <b>증거금(행사가/기초자산×100×40%)</b> 기준<br>
