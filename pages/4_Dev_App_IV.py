...
st.set_page_config(page_title="실제 옵션 체인 기반 전략 추천 (4_Dev_App_IV)", layout="centered")
st.title("실제 옵션 체인 기반 전략 추천 (4_Dev_App_IV)")
...
try:
    ...
    # 2번 기능: 각 행사가별 IV(내재변동성) 테이블 표시
    if 'impliedVolatility' in calls.columns and 'impliedVolatility' in puts.columns:
        iv_df = pd.DataFrame({
            '행사가': strikes,
            '콜 IV': [float(calls[calls['strike'] == s]['impliedVolatility'].values[0]) if len(calls[calls['strike'] == s]['impliedVolatility'].values) > 0 else np.nan for s in strikes],
            '풋 IV': [float(puts[puts['strike'] == s]['impliedVolatility'].values[0]) if len(puts[puts['strike'] == s]['impliedVolatility'].values) > 0 else np.nan for s in strikes],
        })
        st.subheader("행사가별 내재변동성(IV)")
        st.dataframe(iv_df)
    else:
